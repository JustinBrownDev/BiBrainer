...
def addnode(widget):  # is used for adding new neurons
    ...
    nodemode = simpledialog.askinteger(title="Set the Neuron's mode",
                                       prompt="Choose from one of the modes 2-7 (1 is reserved for Input Neurons)") - 1
...
def addsynapse(widget):  # is used for adding new synapses
    ...
    widget.insert(widget.size(),
                  "Synapse: " + "From: " + str(syninput) + " To: " + str(synout) + " Weight: " + str(synweight))
    newsyn = {
        'Inov': 0,
        'NodeIn': syninput,
        'NodeOut': synout,
...
def returnallobjectsinlistdict(list,
                               objectname):  # implementation for returing a list of all n inside lists inside of a list (pretty stupid i know)
    returnlist = []
    for element in list:
...
def openfile():  # used for opening a Bibite File
    ...
    try:
        ...
        messagebox.showerror('Couldn\'t open!',
                             'BiBrainer has failed to open this file. if you think this is a bug please report this (and steps to reproduce) to the Creator!')
        openfile()
...
def main():  # create the tkinter window(and a bunch of other stuff im too lazy to explain)
    ...
    syn_listbox = tk.Listbox(
        window,
        listvariable=None,
        height=14,
        width=33,
        selectmode='browse')
    syn_listbox.grid(row=1, column=2)
    static_neurons = tk.StringVar(value=returnallobjectsinlistdict(json["brain"]["Nodes"][:48], "Desc"))
    hidden_neurons = tk.StringVar(value=returnallobjectsinlistdict(json["brain"]["Nodes"][48:], "Desc"))
    for element in json["brain"]["Synapses"]:
        syn_listbox.insert(syn_listbox.size(), "Synapse: " + "From: " + str(element["NodeIn"]) + " To: " + str(
            element["NodeOut"]) + " Weight: " + str(element["Weight"]))
    static_neuron_label = tk.Label(window, text="Input and Output Neurons")
    static_neuron_label.grid(row=0, column=0)
    syn_label = tk.Label(window, text="Synapses")
    syn_label.grid(row=0, column=1)

    static_neuron_listBox = tk.Listbox(
        window,
        listvariable=static_neurons,
        height=14,
        width=33,
        selectmode='multiple')
    hidden_neuron_listBox = tk.Listbox(
        window,
        listvariable=hidden_neurons,
        height=14,
        width=33,
        selectmode='browse')
    static_neuron_listBox.grid(column=0, row=1)
    hiddentips = tk.Label(window,
                          text="Need Help?: " + "\nhttps://drive.google.com/file/d/1-6AyPj3MMdWPcam4Uq-2ofupDO9vKrZP/view",
                          fg="blue", cursor="hand2")
    hiddentips.grid(row=4, column=2)
    hiddentips.bind("<Button-1>", openhyper)
    hidden_label = tk.Label(window, text="Hidden Neurons")
    hidden_label.grid(row=3, column=0)
    hidden_neuron_listBox.grid(column=0, row=4)

    addbutton = tk.Button(window, text="Add Neuron", command=lambda: addnode(hidden_neuron_listBox))
    addbutton.grid(column=0, row=5)
    addbutton = tk.Button(window, text="Remove selected Synapse", command=lambda: removesyn(syn_listbox))
    addbutton.grid(column=2, row=3)
    addsybutton = tk.Button(window, text="Add Synapse", command=lambda: addsynapse(syn_listbox))
    addsybutton.grid(column=2, row=2)
    addbutton = tk.Button(window, text="Remove selected Neuron", command=lambda: removeneuron(hidden_neuron_listBox))
